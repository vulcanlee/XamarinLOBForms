...
namespace LOBForm.Repositories
{
    public class DepartmentRepository
    {
        ...
        /// 身分驗證成功後的使用者紀錄
        /// </summary>
        public List<DepartmentModel> Items { get; set; } = new List<DepartmentModel>();
                        ...
                        if (response != null)
                        {
                            if(response.IsSuccessStatusCode == true)
                            {
                                ...
                                if (fooAPIResult.Success == true)
                                {
                                    ...
                                    Items = JsonConvert.DeserializeObject<List<DepartmentModel>>(fooAPIResult.Payload.ToString());
                                    if (Items == null)
                                    {
                                        Items = new List<DepartmentModel>();
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<DepartmentModel>> ReadAsync()
        {
            string data = "";
            data = await StorageUtility.ReadFromDataFileAsync("", MainHelper.資料主目錄, MainHelper.DepartmentAPIName);
            Items = JsonConvert.DeserializeObject<List<DepartmentModel>>(data);
            if (Items == null)
            {
                Items = new List<DepartmentModel>();
            }
