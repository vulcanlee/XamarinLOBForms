            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            LoadingMessage = "正在更新 使用者部門名稱清單 中";
                            var fooDepartmentRepository = new DepartmentRepository();
                            APIResult = await fooDepartmentRepository.GetAsync();
                            if (APIResult.Success == false)
                            {
                                using (Acr.UserDialogs.UserDialogs.Instance.Toast(
                                    $"無法進行 更新 使用者部門名稱清單 中({APIResult.Message})", TimeSpan.FromMilliseconds(1500)))
                                {
                                    await Task.Delay(2000);
                                }
                                fooAllSuccess = false;
